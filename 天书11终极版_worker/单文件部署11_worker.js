...
let 哎呀呀这是我的VL密钥 = "047f02b4-a674-4eb5-9f40-5263a2bfbdce"; //这是真实的UUID，通用订阅会进行验证，建议修改为自己的规范化UUID
...
export default {
  async fetch(访问请求, env) {
    ...
    if (!读取我的请求标头 || 读取我的请求标头 !== 'websocket') {
      ...
      我的NAT64的IP = 读取环境变量('NAT64', 我的NAT64的IP, env)
    ...
    try {
      ...
      if (启用反代功能) {
        try {
          ...
          if (启用NAT64功能) {
            let NAT64地址;
            if (/^\d+\.\d+\.\d+\.\d+$/.test(访问地址)) {  // IPv4地址
              NAT64地址 = 转换到NAT64的IPv6(访问地址);
            } else if (访问地址.includes(':')) {  // IPv6地址
              throw new Error('IPv6地址无需转换');
            } else {  // 域名
              NAT64地址 = await 获取IPv6代理地址(访问地址);
            }
            TCP接口 = connect({ hostname: NAT64地址, port: 访问端口 || 443 });
          }
        }
      } else if (启用NAT64功能) {
        try {
